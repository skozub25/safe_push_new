import re
from scanner.patterns import PATTERN_RULES
...
def test_core_patterns_are_present_in_rules():
    sources = {rule.regex.pattern for rule in PATTERN_RULES}
...
def test_azure_service_bus_conn_string_pattern():
    ...
    cs = (
        ...
        "SharedAccessKey=ABCDEF1234567890"
    )
    assert _has_match(pat, cs)
    assert not _has_match(
        pat,
        "Endpoint=https://example;SharedAccessKeyName=x;SharedAccessKey=y",
    )  # not sb://
...
def test_new_patterns_present_in_rules():
    sources = {rule.regex.pattern for rule in PATTERN_RULES}
